		...
		for ( var j = 0; j < methodKeys.length; j += 1 ) {
			...
			for ( var k = 0; k < tableKeys.length; k += 1 ) {
				...
				for ( var m = 0; m < rootFieldKeys.length; m += 1 ) {
					...
					//TODO: выяснить название пути для нахождения положения джойна в нем
					joinPosition = getPathPosition( self._paths[ self._vids ], [service, method, table, rootField] );

					// rootJoinFields[rootField] либо проинициализируется, либо примет новое меньшее значение
					rootJoinFields[rootField] = ((rootJoinFields[rootField] || 999) > joinPosition) ? joinPosition : rootJoinFields[rootField];
				}
			}
		}
	}
