...
//		requestShrinks,
	...
	return async.waterfall( tasks, function( err, job ) {
...
function buildRootJoinFields( job, cb ) {
	...
	for ( var k = 0; k < job.request.vids.length; k += 1 ) {
		dict = self._vids[ job.request.name ][ job.request.vids[k] ];
		...
		if ( !rootField ) { // root
			...
			joinRequest[smtr] = joinRequest[smtr] || { table: table, query: {}, fields: [ self._doc.ID ] };
...
function buildAccessRequest( job, cb ) {
	...
	if ( !self._views[ job.request.name ].access[ME] ) { return next( cb, null, job ); }

	rules = self._views[ job.request.name ].access[ME];
...
function buildJoinQueryAndRequest( job, cb ) {
	...
	var keys, chain, doc, len;
	var tasks = [];
	var req = job.joinRequest;
	var rootFieldPathDistance = {};
	var rootFieldIdsByDistance = {};
	...
	for ( var i = 0; i < keys.length; i += 1 ) {
		...
		// rootFieldPathDistance[rootField] либо проинициализируется, либо примет новое меньшее значение
		rootFieldPathDistance[rootField] = ((rootFieldPathDistance[rootField] || 0) < joinDistance) ? joinDistance : rootFieldPathDistance[rootField];
	}

	job.rootFieldPathDistance = rootFieldPathDistance;

	// разбор полей корня по которым совершаются джойны
	//TODO: надо ли это исправить: id джойнов разных документов корня сливаются в один массив
	keys = Object.keys( rootFieldPathDistance );
	for ( var j = 0; j < keys.length; j += 1 ) {
		len = rootFieldPathDistance[ keys[j] ];
		rootFieldIdsByDistance[ keys[j] ] = [];

		// контейнеры под удаленность
		for ( var d = 0; d < len; d += 1 ) {
			rootFieldIdsByDistance[ keys[j] ][d] = [];
		}

		// наполнение
		for ( var k = 0; k < job.rootResult.length; k += 1 ) {
			doc = job.rootResult[k];
			rootFieldIdsByDistance[ keys[j] ][k] = [];

			for ( var x = 0; x < doc[ keys[j] ].length; x += 1 ) {
				chain = doc[ keys[j] ][x].split( '_' );
				if ( chain.length !== len ) { continue; }

				for ( var y = 0; y < len; y += 1 ) {
					rootFieldIdsByDistance[ keys[j] ][y].push( chain[len - y - 1] );
				}
			}
		}
		...
		for ( var f = 0; f < len; f += 1 ) {
			rootFieldIdsByDistance[ keys[j] ][f] = unique( rootFieldIdsByDistance[ keys[j] ][f] );
		}
	}
	...
	return async.map( tasks, function( task, cb ) {
		...
		request.request.fields = unique( request.request.fields );
		request.request.query[ self._doc.ID ] = { $in: rootFieldIdsByDistance[task.rootField][task.joinDistance - 1] };
		...
		for ( var i = 0; i < res.length; i += 1 ) {
			job.joinResult[ tasks[i].smtr ] = tasks[i];
			job.joinResult[ tasks[i].smtr ].data = res[i];
		}
...
function buildLinks( job, cb ) {
	var self = job.self;
	var service, method, table, rootField, path, smtr;
	var keys;
	var joinData, joinPosition, parentPosition, parentData;
	job.links = [];
	job.joins = [];
	...
	var usedJoins = {};
	keys = Object.keys( job.rootFieldPathDistance );
	for ( var i = 0; i < keys.length; i += 1 ) {
		parentPosition = 0;
		rootField = keys[i];
		path = pathByRootField[ rootField ].path;
		var distance = job.rootFieldPathDistance[ rootField ];
		for ( var j = 0; j < distance; j += 1 ) {
			var elementPosition = pathByRootField[rootField].position - j;

			var element = self._paths[path][elementPosition];
			if ( !element ) { break; } //FIXME

			smtr = [element[0], element[1], element[2], element[3]].join( '/' );
			if ( usedJoins[smtr] !== undefined ) {
				joinPosition = usedJoins[smtr];
				joinData = job.joins[joinPosition];
			} else {
				joinData = job.joinResult[ smtr ];
				joinPosition = job.joins.push( joinData ) - 1;
			}

			if ( !joinData ) { break; } // FIXME

			for ( var k = 0; k < job.rootResult.length; k += 1 ) {
				for ( var z = 0; z < job.rootResult[k][rootField].length; z += 1 ) {
					var chain = job.rootResult[k][rootField][z].split( '_' );
					if ( chain.length !== distance ) { continue; }

					for ( var d = 0; d < joinData.length; d += 1 ) {
						if ( joinData[d]._id !== chain[distance - 1 - j] ) { continue; }
						
						job.joins.push( [
							0, // parent container position
							k, // root document
							joinPosition, // child container position
							d // join document
						] );
					}
				}
...
function buildResponse( job, cb ) {
	...
	job.response = {
		links: job.links,
		data: [job.rootResult, job.joinResult] //FIXME
	};
	return next( cb, null, job );
}
...
//     ['service', 'method', 'table', 'field']
// ]
// elem = ['service', 'method', 'table', 'field']
