...
//TODO: поддержка звездочек (wildcards) в селекторах
...
function applyRule( rule, trOut, trIn ) {
	...
	if ( !ruleValue.length ) { throw new Error( 'Empty rule value on key `' + ruleKey.join( '.' ) + '`' ); }

	// исполнение спецкоманды
	if ( specials[ ruleValue[0] ] ) {
		if ( !ruleValue.substr( 1 ).length ) { throw new Error( 'Empty command on key `' + ruleKey.join( '.' ) + '`' ); }
...
var commands = {
	del: function( ruleKey, ruleValue, trOut, trIn ) { // удаление узла
		var parent = ruleKey.slice( 0, -1 );
		if ( !trOut.has( parent ) ) { return; }

		delete trOut.get( parent )[ ruleKey[ruleKey.length - 1] ];
	}
};
...
function getPathArray( path ) {
	if ( !path.length ) { throw new Error( 'Empty rule key' ); }
	return path.split( '.' );
}
