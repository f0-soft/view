...
function findRootCallback( err, results, count ) {
	...
	for ( var i = 0; i < this.vids.length; i += 1 ) {
		if ( VIEWS[this.name].aggr[this.vids[i]] !== undefined ) {
			...
			if ( taskContainer[aggr.aggregate] === undefined ) {
				taskContainer[aggr.aggregate] = {
					group: { _id: '0' },
					match: {}
				};
			}
			if ( aggr.group ) {
				taskContainer[aggr.aggregate].group[this.vids[i]] = aggr.group;
			}
		}
	}
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		aggr = VIEWS[this.name].aggr[keys[i]];
		if ( aggr !== undefined && taskContainer[aggr.aggregate] !== undefined ) {
			taskContainer[aggr.aggregate].match[aggr.property] = this.request[keys[i]];
		}
	}
		...
		for ( var j = 0; j < results.length; j += 1 ) {
			...
			if ( link_key === DOC_PATH ) {
				...
				for ( k = 0; k < link_subkeys.length; k += 1 ) {
					...
					if ( link[link_key][link_subkeys[k]] === '%id%' ) {
