...
function findShrinksCallback( err, data ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		if ( !request[names[1]] ) {
			...
			if ( request_in.length === 0 ) { // no documents
				return this.callback( null, {result: [], count: 0} );
			}
		}
	}
	...
	if ( VIEWS[this.name].view.access && VIEWS[this.name].view.access[this.access.role] ) {
		var access = VIEWS[this.name].view.access[this.access.role];

		keys = Object.keys( access );
		for ( i = 0; i < keys.length; i += 1 ) {
			this.rootSelector[ keys[0] ] = access[ keys[0] ].replace( REPLACE_USER_ID, this.access.user_id ).replace( REPLACE_COMPANY_ID, this.access.company_id );
		}
	}



	if ( this.request.options && this.request.options.sort ) {
		keys = Object.keys( this.request.options.sort );
		...
		if ( keys.length === 1 ) {
			...
			sortOption[2][ sort_vid[1] ] = this.request.options.sort[ keys[0] ];
		}
	}

	var requestOptions = {};
	keys = Object.keys( this.request.options );
	for ( i = 0; i < keys.length; i += 1 ) {
		requestOptions[keys[i]] = this.request.options[keys[i]];
	}
...
function findRootCallback( err, data ) {
	...
	if ( data.result.length === 0 ) {
		return this.callback( null, {result: [], count: data.count} );
	}
	...
	// gather grouping
	// TODO: сделать группировку на столбец, а не на ячейку
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		for ( var j = 0; j < data.result.length; j += 1 ) {
			...
			if ( link_key !== DOC_ID ) { pipeline.push( { $unwind: '$' + link_key } ); }
...
function findAggregateCallback( err, results ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		var alias = PROVIDER_ALIAS[elem.flexo];
...
function findJoinsCallback( err, result ) {
	...
	var i, j, k, x, keys, roots, chains, res = [];

	if ( err ) { return this.callback( err ); }

	var ReverseProviderAlias = {};
	keys = Object.keys( PROVIDER_ALIAS );
	for ( i = 0; i < keys.length; i += 1 ) {
		ReverseProviderAlias[ PROVIDER_ALIAS[keys[i]] ] = keys[i];
	}
	...
	for ( i = 0; i < paths.length; i += 1 ) {
		...
		for ( j = 0; j < roots.length; j += 1 ) {
			...
			var max = 0;
			chains = [];
			for ( k = 0; k < roots[j][ usedPaths[paths[i]] ].length; k += 1 ) {
				var splt = roots[j][ usedPaths[paths[i]] ][k].split( '_' );
				if ( splt.length > max ) {
					max = splt.length;
					chains = [];
				}
			...
			for ( k = 0; k < chains.length; k += 1 ) {
				...
				for ( x = chain.length - 1; x >= 0; x -= 1 ) {
					...
					flexo = ReverseProviderAlias[alias];
					var key = '' + flexo + '|' + field;
					var index = this.tasks_dict.indexOf( key );
						...
						for ( var y = 0; y < result[index].result.length; y += 1 ) {
							if ( result[index].result[y][DOC_ID] === chain[x] ) {
								if ( result[index].result[y][VID_LINK] === undefined ) {
									// конвертация в vid
									var cont = {};
									for ( var z = 0; z < this.vids.length; z += 1 ) {
										if ( VIEWS[this.name].vids[this.vids[z]] && VIEWS[this.name].vids[this.vids[z]][0] === flexo && VIEWS[this.name].vids[this.vids[z]][1] !== undefined && VIEWS[this.name].vids[this.vids[z]][2] === field ) {
											cont[this.vids[z]] = result[index].result[y][ VIEWS[this.name].vids[this.vids[z]][1] ];
										}
									}
								...
								if ( writeInJoin === false ) {
									if ( !res[0][j][VID_LINK] ) { res[0][j][VID_LINK] = []; }
									res[0][j][VID_LINK].push( result[index].result[y][VID_LINK] );
									writeInJoin = result[index].result[y][VID_LINK];
								} else {
									if ( !res[1][writeInJoin][VID_LINK] ) { res[1][writeInJoin][VID_LINK] = []; }
									res[1][writeInJoin][VID_LINK].push( result[index].result[y][VID_LINK] );
								}
							}
						}
					}
			...
			for ( j = 0; j < roots.length; j += 1 ) {
				for ( k = 0; k < result[i].result.length; k += 1 ) {
					if ( result[i.result][k][DOC_ID] === roots[j][DOC_ID] ) {
						...
						if ( !res[0][j][VID_LINK] ) { res[0][j][VID_LINK] = []; }
						res[0][j][VID_LINK].push( result[i].result[k][VID_LINK] );
					}
				}
			}
		}
...
container.insert = function( name, vids, request, access, callback ) {
	...
	if ( VIEWS[name].view.access && VIEWS[name].view.access[access.role] ) {
		my_access = VIEWS[name].view.access[access.role];
		keys = Object.keys( my_access );
		for ( i = 0; i < keys.length; i += 1 ) {
			if ( my_access[keys[i]] === REPLACE_USER_ID || my_access[keys[i]] === REPLACE_COMPANY_ID ) {
				insertion = [keys[i], my_access[keys[i]].replace( REPLACE_USER_ID, access.user_id ).replace( REPLACE_COMPANY_ID, access.company_id )];
				break;
			}
		}
	}
	...
	return PROVIDER.insert( {name: rootName, fields: rootFields, documents: documents, options: {}}, insertCallback.bind( {name: name, vids: myVids, callback: callback} ) );
};
