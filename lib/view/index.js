...
function findShrinksCallback( err, data ) {
	...
	if ( options.sort ) {
		...
		if ( keys.length === 1 ) {
			...
			sortOption = [
				sort_vid[0], // сортируемая коллекция
				(sort_vid[0] !== rootName) ? sort_vid[2] : '', // поле связи или пусто
...
function findAggregateCallback( err, results ) {
		...
		if ( flexo !== rootName ) {
			if ( !joinProps[key] ) {
				joinProps[key] = {
					...
					fields: [],
					vids: [],
					documents: this.rootData.result
				};
			}
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		if ( this.rootData.result.length ) {
			isarray = Array.isArray( this.rootData.result[0][elem.field] )
		}
		ids = [];
		for ( j = 0; j < this.rootData.result.length; j += 1 ) {
			if ( isarray ) {
				for ( k = 0; k < this.rootData.result[j][elem.field].length; k += 1 ) {
					if ( this.rootData.result[j][elem.field][k].substr( 0, alias.length ) === alias ) {
						ids.push( this.rootData.result[j][elem.field][k].split( '_', 1 )[0] );
					}
				}
			} else {
				if ( this.rootData.result[j][elem.field] !== undefined ) {
					ids.push( this.rootData.result[j][elem.field] );
				}
			}
		}
...
function findJoinsCallback( err, result ) {
	...
	var i, j, k, z, x, key, index, roots, chains, target, res = [];
	...
	for ( i = 0; i < this.vids.length; i += 1 ) {
		if ( VIEWS[ this.name ].aggr[ this.vids[i] ] === undefined ) { continue; }
		if ( VIEWS[ this.name ].aggr[ this.vids[i] ].group === undefined ) { continue; }

		for ( j = 0; j < roots.length; j += 1 ) {
			res[0][j][ this.vids[i] ] = 0;
			for ( k = 0; k < this.aggregate.length; k += 1 ) {
				if ( roots[j][DOC_ID] !== this.aggregate[k][DOC_ID] ) { continue; }
				if ( this.aggregate[k][ this.vids[i] ] === undefined ) { continue; }
				res[0][j][ this.vids[i] ] = this.aggregate[k][ this.vids[i] ];
				break;
			}
		}
	}

	this.rootData.result = res;
	delete this.rootData.idFields;


	return this.callback( null, this.rootData );
}
