...
container.getTemplate = function( name, vids, callback ) {
	...
	if ( VIEWS[name] === undefined ) {
		return next( callback, myErr( 'There\'s no view ' + name ) );
	}
...
container.find = function( name, vids, request, options, callback ) {
	...
	if ( VIEWS[name].view.root === undefined ) {
		return next( callback, myErr( 'Request to view without root' ) );
	}
	...
	for ( i = 0; i < vids.length; i += 1 ) {
		flexo = VIEWS[ name ].vids[ vids[i] ][0];
		prop = VIEWS[ name ].vids[ vids[i] ][1];
		field = VIEWS[ name ].vids[ vids[i] ][2];
		path = VIEWS[ name ].vids[ vids[i] ][3];
	...
	return async.parallel( tasks, findShrinksCallback.bind( { name: name, rootSelector: rootSelector, rootFields: rootFields, requestOptions: requestOptions, vids: vids, options: options, paths: paths, callback: callback } ) );
};
...
function findRootCallback( err, results, count ) {
	...
	// translate properties to vids, except _path
	rootName = VIEWS[ this.name ].view.root;
...
function findJoinsCallback( err, result ) {
	...
	pathField = {};
	for ( i = 0; i < this.vids.length; i += 1 ) {
		if ( VIEWS[this.name].vids[this.vids[i]][3] !== undefined ) {
	...
	for ( i = 0; i < usedPaths.length; i += 1 ) {
		...
		// gather starting ids
		old_ids = [];
		for ( j = 0; j < this.documents[0].length; j += 1 ) {
			old_ids[j] = Array.isArray( this.documents[0][j][field] ) ? this.documents[0][j][field] : [ this.documents[0][j][field] ];
		}
	...
	this.documents[0] = vidlib.nestVidsFromData( rootVids, VIEWS[this.name].vids, this.documents[0] );
