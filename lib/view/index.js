...
function findJoinsCallback( err, result ) {
	...
	var i, j, k, z, x, keys, roots, chains, res = [];
					...
					if ( index !== -1 ) {
						...
						if ( !result[index].result.length ) {
							...
							if ( writeInJoin === false ) {
								...
								if ( res[0][j][VID_LINK].indexOf( result[index].empty ) === -1 ) {
									res[0][j][VID_LINK].push( result[index].empty );
								}
								writeInJoin = result[index].empty;
							} else {
								if ( !res[1][writeInJoin][VID_LINK] ) { res[1][writeInJoin][VID_LINK] = []; }
								if ( res[1][writeInJoin][VID_LINK].indexOf( result[index].empty ) === -1 ) {
									res[1][writeInJoin][VID_LINK].push( result[index].empty );
								}
							}
							...
							for ( var y = 0; y < result[index].result.length; y += 1 ) {
								if ( result[index].result[y][DOC_ID] === chain[x] ) {
									...
									if ( writeInJoin === false ) {
										if ( !res[0][j][VID_LINK] ) { res[0][j][VID_LINK] = []; }
										if ( res[0][j][VID_LINK].indexOf( result[index].result[y][VID_LINK] ) === -1 ) {
											res[0][j][VID_LINK].push( result[index].result[y][VID_LINK] );
										}
										writeInJoin = result[index].result[y][VID_LINK];
									} else {
										if ( !res[1][writeInJoin][VID_LINK] ) { res[1][writeInJoin][VID_LINK] = []; }
										if ( res[1][writeInJoin][VID_LINK].indexOf( result[index].result[y][VID_LINK] ) === -1 ) {
		...
		if ( usedJoins[this.tasks_dict[i]] === undefined ) {
			...
			for ( j = 0; j < roots.length; j += 1 ) {
				...
				if ( !result[i].result.length ) {
					...
					if ( !res[0][j][VID_LINK] ) { res[0][j][VID_LINK] = []; }
					if ( res[0][j][VID_LINK].indexOf( result[i].empty ) === -1 ) {
						res[0][j][VID_LINK].push( result[i].empty );
					}
					...
					for ( k = 0; k < result[i].result.length; k += 1 ) {
						if ( ids.indexOf( result[i].result[k][DOC_ID] ) !== -1 ) {
							...
							if ( !res[0][j][VID_LINK] ) { res[0][j][VID_LINK] = []; }
							if ( res[0][j][VID_LINK].indexOf( result[i].result[k][VID_LINK] ) === -1 ) {
