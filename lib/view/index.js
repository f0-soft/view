...
function findShrinksCallback( err, result ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		if ( pathRootPos - pathJoinPos > 1 ) {
			...
			and_elem = {};
			and_elem[DOC_PATH] = {$elemMatch: { c: path[pathJoinPos + 1][0], f: names[1], o: {$in: ids} }};
			request.$and.push( and_elem );
		} else {
			if ( !request[ names[1] ] ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		if ( this.rootSelector[ keys[i] ] !== undefined ) {
			...
			// find common
			for ( j = this.rootSelector[ keys[i] ].$in.length - 1; j > 0; j -= 1 ) { // except zero element with '$in'
				if ( request[ keys[i] ].$in.indexOf( this.rootSelector[ keys[i] ].$in[j], 1 ) === -1 ) {
					this.rootSelector[ keys[i] ].$in.splice( j, 1 );
				}
			}

			if ( this.rootSelector[ keys[i] ].$in.length === 0 ) { // no documents
				return this.callback( null, [
					[]
				], 0 );
			}
	...
	if ( VIEWS[this.name].view.access && VIEWS[this.name].view.access[this.options.role] ) {
		var access = VIEWS[this.name].view.access[this.options.role];

		keys = Object.keys( access );
		if ( keys.length > 0 ) {
			if ( keys[0] === DOC_PATH ) {
				if ( access[DOC_PATH].$elemMatch ) {
					insertion = { $elemMatch: {}};

					// clone request
					subkeys = Object.keys( access[DOC_PATH].$elemMatch );
					for ( i = 0; i < subkeys.length; i += 1 ) {
						if ( access[DOC_PATH].$elemMatch[ subkeys[i] ] === REPLACE_USER_ID ) {
							insertion.$elemMatch[ subkeys[i] ] = this.options.user_id;
						} else if ( access[DOC_PATH].$elemMatch[ subkeys[i] ] === REPLACE_COMPANY_ID ) {
							insertion.$elemMatch[ subkeys[i] ] = this.options.company_id;
						} else {
							insertion.$elemMatch[ subkeys[i] ] = access[DOC_PATH].$elemMatch[ subkeys[i] ];
						}
					}


					if ( !this.rootSelector.$and ) {
						this.rootSelector.$and = [];
					}

					if ( this.rootSelector[DOC_PATH] ) {
						path = {};
						path[DOC_PATH] = this.rootSelector[DOC_PATH];
						this.rootSelector.$and.push( path );
						delete this.rootSelector[DOC_PATH];
					}

					path = {};
					path[DOC_PATH] = insertion;
					this.rootSelector.$and.push( path );
				}
			} else {
				this.rootSelector[ keys[0] ] = access[ keys[0] ].replace( REPLACE_USER_ID, this.options.user_id ).replace( REPLACE_COMPANY_ID, this.options.company_id );
			}
		}
	}
	...
	var sortByJoin = false;
	var sortOut = {};
	if ( this.requestOptions.sort ) {
		keys = Object.keys( this.requestOptions.sort );
		var sort = this.requestOptions.sort;

		if ( keys.length === 0 ) { return this.callback( myErr( 'когда указана опция сортировки, она должна содержать хотя бы 1 ключ' ) );}
		...
		if ( VIEWS[this.name].vids[keys[0]][0] === rootName ) {
			sortOut[VIEWS[this.name].vids[keys[0]][1]] = sort[keys[0]];
			this.requestOptions.sort = sortOut;
		} else {
			// запрос к джойн-коллекции для поиска
			return this.callback( myErr( 'сортировка по присоединяемым полям пока недоступна' ) );
//			return PROVIDER.find( rootName, [DOC_ID, DOC_PATH], {selector: selectorObjectToArray( {} ), options: {sort: sortOut}}, {sort: sortOut}, findRootForJoinSortCallback.bind( this ) );
		}
	}
	...
	return PROVIDER.find( rootName, this.rootFields, {selector: selectorObjectToArray( this.rootSelector ), options: this.requestOptions}, this.requestOptions, findRootCallback.bind( this ) );
}

function findRootForJoinSortCallback( err, result, count ) {
	// gather ids of joins in roots
	// make an aggregation request with match, sort, skip, limit, unwind, match, group, skip, limit, fuck
}
...
function findRootCallback( err, results, count ) {
	...
	for ( i = 0; i < keys.length; i += 1 ) {
		...
		for ( var j = 0; j < results.length; j += 1 ) {
			...
			tasks.push( PROVIDER.aggregate.bind( PROVIDER, coll, [
				{ $match: match },
				{ $group: group }
			] ) );
		}
...
container.insert = function( name, vids, request, options, callback ) {
	...
	// insert user/company identification
	if ( VIEWS[name].view.access && VIEWS[name].view.access[options.role] ) {
		access = VIEWS[name].view.access[options.role];
		keys = Object.keys( access );
		if ( keys.length > 0 && keys.indexOf( DOC_ID ) === -1 && keys.indexOf( DOC_PATH ) === -1 ) {
			insertion = [keys[0], access[keys[0]].replace( REPLACE_USER_ID, options.user_id ).replace( REPLACE_COMPANY_ID, options.company_id )];
		}
	}
	...
	for ( i = 0; i < request.length; i += 1 ) {
		...
		if ( insertion ) {
			documents[i][ insertion[0] ] = insertion[1];
		}
	}
...
container.modify = function( name, request, options, callback ) {
	...
	return PROVIDER.modify( rootName, flexoQuery, options, modifyCallback.bind( { name: name, vids: [vid_id, vid_update], callback: callback} ) );
};
