...
function findShrinksCallback( err, result ) {
	...
	if ( this.requestOptions.sort ) {
		...
		for ( i = 0; i < this.requestOptions.sort.length; i += 1 ) {
			if ( VIEWS[this.name].vids[sort[i][0]] === undefined ) { continue; }
			if ( VIEWS[this.name].vids[sort[i][0]][0] === undefined ) { continue; }
			if ( VIEWS[this.name].vids[sort[i][0]][1] === undefined ) { continue; }
			if ( VIEWS[this.name].vids[sort[i][0]][0] !== rootName ) { continue; }
			sortOut.push( [ VIEWS[this.name].vids[sort[i][0]][1], sort[i][1] ] );
		}
