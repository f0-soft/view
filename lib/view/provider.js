...
Provider.prototype.start = function( cb ) {
	this._connector.start( cb );
};
...
Provider.prototype.request = function( request, cb ) {
	...
	self._connector.request( {
		...
		proxyResponse = argstrans( responseRules, args, responseTemplate );

		return cb.apply( null, proxyResponse );
	} );
};
