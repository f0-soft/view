...
exports.vidByProp = function( prop, vids, dict ) {
	...
	for ( i = 0; i < vids.length; i += 1 ) {
		if ( dict[ vids[i] ] && dict[ vids[i] ][1] === prop ) {
			return vids[i];
		}
	}
...
exports.vidsOfFlexo = function( dict, vids, flexo, field ) {
	...
	if ( field !== undefined ) {
		for ( i = 0; i < vids.length; i += 1 ) {
			if ( dict[vids[i]] && dict[vids[i]][0] === flexo && dict[vids[i]][2] === field ) {
				res.push( vids[i] );
			}
		}
	} else {
		for ( i = 0; i < vids.length; i += 1 ) {
			if ( dict[vids[i]] && dict[vids[i]][0] === flexo ) {
				res.push( vids[i] );
			}
		}
	}
...
exports.fieldsOfFlexo = function( flexo, vids, dict ) {
	var i, res = [];
	for ( i = 0; i < vids.length; i += 1 ) {
		if ( dict[vids[i]] && dict[vids[i]][0] === flexo && res.indexOf( dict[vids[i]][1] ) === -1 ) {
